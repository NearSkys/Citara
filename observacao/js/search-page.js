...
async function performSearch(query, page = 1) {
  ...
  // SAVE RESULTS TO CURRENT PROJECT
  if (Storage.currentProject) {
    Storage.currentProject.searchResults = {
      query: currentQuery,
      timestamp: Utils.formatDateTime(),
      results: paperRes.items || [] // Saving only papers for now as main result
    };
    Storage.updateCurrentProject();
  }
...
// Optional: trigger initial search if query param exists OR load from storage
(function initFromURL() {
  try {
    ...
    if (q) {
      input.value = q;
      performSearch(q, 1);
    } else if (Storage.currentProject && Storage.currentProject.searchResults && Storage.currentProject.searchResults.query) {
      // Load from storage
      const saved = Storage.currentProject.searchResults;
      input.value = saved.query;
      // We can either re-run search or render saved results.
      // Re-running ensures freshness, but rendering saved is faster/offline-friendly.
      // Let's re-run for now to be safe, or just render if we want strict persistence.
      // Given "no processing" requirement, maybe rendering saved is better?
      // But performSearch handles rendering. Let's just call performSearch with the saved query.
      performSearch(saved.query, 1);
    }
  } catch (e) {
    // ignore
  }
